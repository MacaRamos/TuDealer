...
(function($) {
    $.fn.extend({
		...
		setZoomPicture: function(options) {	
			//DÃ©claration des paramÃ¨tres par dÃ©faut
			var defaults = {
			...
			// Initialisation de la collection de paramÃ¨tres
			var _options = $.extend(defaults, options);
			
			// CrÃ©ation de la classe de gestion
			var diaporama = {
				_opt: null,
				_totalImg: 0,
				_currentImg: 0,
				...
				init: function(opt) {						
					...
					// Initialisation des Ã©vÃ©nements 'click' sur les miniatures et les boutons
					this._attachThumbsEvent();

					// Initialisation des effets de zoom
					this._setZoomEffects();

					// Affichage de la premiÃ¨re image de la banque
				...
				_attachThumbsEvent: function() {
					...
					$(this._opt.thumbsContainer).find("img").click(function(e) {
						...
						// Remplacement de l'image affichÃ© par la nouvelle
			...
			return this.each(function() {
				// Ajoute l'objet jQuery courant Ã  la collection 'options'
				_options.jObject = $(this);
				
				// CrÃ©ation d'un message d'attente pendant le chargement
				$(this).find('img').hide();
				...
				// Chargement de la classe diaporama a la fin du tÃ©lÃ©chargement
